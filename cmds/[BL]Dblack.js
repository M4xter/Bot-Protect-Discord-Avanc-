...
module.exports.run = async (bot, message, args, msg) => {
    ...
    if(!message.member.hasPermission("ADMINISTRATOR")) return message.reply(":x: Vous n'avez pas la permission ! [ADMINISTRATOR]");
    
    var user = message.mentions.members.first() || message.guild.members.cache.get(args[0]);
    if(!user) return message.reply(':x: Veuillez mentionner une personne ou son ID !');

    let reason = args[1];
    if(!reason) return message.reply(':x: Veuillez spécifier une raison & une preuve !');
